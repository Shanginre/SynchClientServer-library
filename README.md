...
Сервер TCP может работать с произвольным количество соединений как на одном, так и на разных портах (что не позволет делать 
...
Задает параметры работы сервера, но не запускает его. Если параметры сервера указаны не корретно, то выдается ошибка.
...
Записывает в переменную ВходящиеСообщенияJson строку JSON с данными о принятых сообщениях, которые не были еще обработанны. 
...
будут асинхронно удалены из памяти компонеты заданием, выполняемым с периодичностью memoryCleaningFrequency.
...
- clientsConnections (массив)
    ...
    - portNumber (число) - номер порта.
    - accepted (булево) - признак подключения клиента к сокету. ИСТИНА - клиент подключен к сокету, ЛОЖЬ - ни один клиен
...
7. После проверки pull реквеста, он либо будет отправлен Вам на доработку, или принят в master ветку. 
