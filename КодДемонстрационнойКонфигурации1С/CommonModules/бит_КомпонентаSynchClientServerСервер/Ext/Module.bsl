...
Функция ПолучитьПараметрыРаботыСервера() Экспорт	
	...
	ПараметрыРаботыСервера = Новый Структура(
		"ПараметрыКомпонентыСервера, СоответствиеПортыТипыКлиентов, СоответствиеТипыАнализаторовМетодики", 
		ПараметрыКомпонентыСервера, Неопределено, Неопределено
	);
...
Функция ПолучитьИнформациюОКомпоненте() Экспорт
	
	ИнформацияСтруктура = Новый Структура(
		"Версия, GithubURL", 
		ПолучитьВерсиюКомпоненты(),
		ПолучитьGithubURLКомпоненты()
	);
...
Функция ПолучитьВерсиюКомпоненты()
	Возврат "1.2";	
КонецФункции

Функция ПолучитьGithubURLКомпоненты()
	Возврат "https://github.com/Shanginre/AddInNative_SynchClientServer";	
КонецФункции
